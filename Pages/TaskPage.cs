...
public sealed class TaskPage : WizardPage
{
    ...
    public TaskPage()
    {
        ...
        layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));
        layout.Controls.Add(_txtTaskName, 0, r++);
        layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));
    ...
    {
        // Auto-populate task name from target if empty
        if (string.IsNullOrWhiteSpace(cfg.TaskName) && !string.IsNullOrWhiteSpace(cfg.TargetName))
            cfg.TaskName = TaskNameHelper.Sanitize($"ProcDump Monitor - {cfg.TargetName}");
